...
interface DesignBlogListProps {
  postsByPage: MyPost[]
  numberOfPage: number
  allTags: TagType[]
}

const DesignBlogList = ({ postsByPage, numberOfPage, allTags }: DesignBlogListProps) => {
...
export default DesignBlogList

export const getStaticProps: GetStaticProps = async (context) => {
  const currentPage = context.params?.page
  const postsByPage =
    typeof currentPage === 'string'
      ? await getPostsByGenreAndPage('Design', parseInt(currentPage.toString(), 10))
      : []
  const numberOfPage = await getNumberOfPagesByGenre('Design')
